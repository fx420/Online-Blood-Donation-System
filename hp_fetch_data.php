...
while($row = mysqli_fetch_assoc($query))
{
	...
	if($row['image'] != ''){
		$sub_array[] = '<center><img style="width:80px;" src="assets/img/upload/healthcare_professional/'.$row['image'].'" /></center> ';
	}else{

		$sub_array[] = '<center>-</center>';
	}
	$sub_array[] = '<a href="javascript:void();" data-id="'.$row['healthcarep_id'].'" class="btn btn-info btn-sm editbtn" ><i class="fa-solid fa-pen-to-square"></i></a>  <a href="javascript:void();" data-id="'.$row['healthcarep_id'].'"  class="btn btn-danger btn-sm deleteBtn" ><i class="fa-solid fa-trash"></i></a>';
	$data[] = $sub_array;
}
